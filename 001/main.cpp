...
void UpdateCache(Window::Data *data) {
	...
	static float pos_y = 2.0;
	static float pos_z = 2.0;
	...
	buf_index = 0;
	for(int i = 0 ; i < 12; i++) {
		auto v0 = projview * glm::vec4(*pcube++, 1.0f);
		auto v1 = projview * glm::vec4(*pcube++, 1.0f);
		auto v2 = projview * glm::vec4(*pcube++, 1.0f);
